(function(){
  ...
  document.querySelectorAll('.toggle-pin').forEach(btn=>{
    btn.addEventListener('click', async (e)=>{
      ...
      await fetch(BASE + '/api/notes_api.php', { method:'POST', headers:{ 'X-CSRF-Token': csrf }, body: form });
      location.reload();
    });
  });
  ...
  document.querySelectorAll('.toggle-fav').forEach(btn=>{
    btn.addEventListener('click', async (e)=>{
      ...
      const id = btn.getAttribute('data-id');
      const csrf = document.querySelector('meta[name="csrf-token"]').content;
      await fetch(BASE + '/actions/toggle_favorite.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-CSRF-Token': csrf }, body: `id=${encodeURIComponent(id)}` });
      location.reload();
    });
  });
  ...
  const doSave = debounce(async ()=>{
    ...
    const res = await fetch(BASE + '/actions/save_note.php', { method: 'POST', headers: { 'X-CSRF-Token': csrfToken }, body: form });
